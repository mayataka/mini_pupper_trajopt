#include "mini_pupper_pacing_controller.hpp"
...
MiniPupperPacingController::MiniPupperPacingController()
  : joint_handlers_(),
    q_cmd_() {
  const std::string path_to_log
      = ros::package::getPath("mini_pupper_trajopt") + "/rsc/pacing/q.log";
  std::ifstream log;
...
bool MiniPupperPacingController::init(
...
void MiniPupperPacingController::starting(const ros::Time& time) {
}


void MiniPupperPacingController::stopping(const ros::Time& time) {
}


void MiniPupperPacingController::update(const ros::Time& time, 
...
PLUGINLIB_EXPORT_CLASS(mini_pupper_trajopt::MiniPupperPacingController, 
