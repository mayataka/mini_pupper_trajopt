#include "mini_pupper_jumping_controller.hpp"
...
MiniPupperJumpingController::MiniPupperJumpingController()
  : joint_handlers_(),
    q_cmd_() {
  const std::string path_to_log
      = ros::package::getPath("mini_pupper_trajopt") + "/rsc/jumping/q.log";
  std::ifstream log;
...
bool MiniPupperJumpingController::init(
...
void MiniPupperJumpingController::starting(const ros::Time& time) {
}


void MiniPupperJumpingController::stopping(const ros::Time& time) {
}


void MiniPupperJumpingController::update(const ros::Time& time, 
...
PLUGINLIB_EXPORT_CLASS(mini_pupper_trajopt::MiniPupperJumpingController, 
