...
class RunningOCPSolverFactory:
    ...
    def create_ocp_solver(self):
        ...
        vcom_ref = np.zeros(3)
        vcom_ref[0] = self.step_length / self.running_time
        com_ref = robotoc.PeriodicCoMRef(com_ref0, vcom_ref, self.t0, 
                                         self.running_time, 0., True)
        com_cost = robotoc.TimeVaryingCoMCost(robot, com_ref)
        com_weight = np.full(3, 1.0e05)
        com_weight[2] = 0.
        com_cost.set_com_weight(com_weight)
        cost.push_back(com_cost)

        # create the constraints
        constraints           = robotoc.Constraints(barrier=1.0e-01)
        ...
        contact_status_front_swing.set_contact_placements(contact_points)
        ...
        contact_status_hip_swing.set_contact_placements(contact_points)
        ...
        for i in range(self.cycle-2):
            ...
            contact_status_flying.set_contact_placements(contact_points)
